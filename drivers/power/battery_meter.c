...
//heyong test auto power
kal_int32 auto_power= 0;
kal_int32 battery_meter_get_battery_percentage(void)
{
	...
	bm_print(BM_LOG_CRTI, "[heyong ppppppppp] %d,%d \r\n",  oam_d_5,oam_d_2);
	//if(auto_power)
	// bm_print(BM_LOG_CRTI, "[heyong ssssssssss] %d ,%d\r\n", g_boot_mode, oam_d_5);
	/*if(g_boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT || g_boot_mode == LOW_POWER_OFF_CHARGING_BOOT)
	{
	if((bat_is_charger_exist() == KAL_TRUE)&&(auto_power == 0)&&(oam_d_2<=95))
		{
		  auto_power = 1;
	         arch_reset(0,NULL);
		}
